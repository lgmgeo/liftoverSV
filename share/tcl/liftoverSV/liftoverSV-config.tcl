...
proc configureLiftoverSV {argv} {
    ...
    ## OUTPUTFILE extension must be ".vcf" (or ".vcf.gz" or ".sort.vcf" or ".sort.vcf.gz")
    ...
    regsub "\\.sort\\.vcf" $g_liftoverSV(OUTPUTFILE) ".vcf" g_liftoverSV(OUTPUTFILE)
    ...
    if {[regexp "/" $g_liftoverSV(OUTPUTFILE)]} {
        set g_liftoverSV(OUTPUTDIR) [file dirname $g_liftoverSV(OUTPUTFILE)]
        if {![file exists $g_liftoverSV(OUTPUTDIR)]} {
            puts "\n############################################################################"
            puts "Bad option value: --OUTPUTFILE = $g_liftoverSV(OUTPUTFILE)"
            puts "Directory do not exist ($g_liftoverSV(OUTPUTDIR))"
            puts "Exit with error."
            puts "############################################################################\n"
            exit 2
        }
